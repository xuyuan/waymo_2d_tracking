...
def lovasz_bce_with_logits(logits, labels, per_image=True, ignore=None):
    error_func = functools.partial(F.binary_cross_entropy_with_logits, reduction='none')
    return lovasz_loss(logits, labels, error_func=error_func, per_image=per_image, ignore=ignore)
...
class LovaszLoss(torch.nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        return lovasz_softmax(probas, targets, per_image=self.per_image)